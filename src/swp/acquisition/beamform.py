...
def build_beamform_pipeline(num_patches: int, is_baseband: bool = False,
    ...
    Args:
        ...
        enable_pfield (bool): weight each transmit by its simulated transmit
            ...
            buffer benefits; see that field for the measurements. Ignored when
            ``refocus`` is set, since ``Refocus`` rewrites the transmit
            parameters and the per-transmit weights no longer mean anything.
    ...
    jit = None if refocus and refocus != "adjoint" else _jit_options()
    return zea.Pipeline(operations, with_batch_dim=True, jit_options=jit)
...
def beamform_frames(raw, params, max_oom_retries=5, enable_pfield=False, refocus=None,
    ...
    # memory budget has to be planned for n_el. Budgeting for n_tx OOMs (73 -> 80 here).
    n_tx_effective = n_el if refocus else n_tx
...
def process_bmode_buffer(vf, spec, out_dir, grid, fps, stem, compression,
    ...
    print(f"  buffer {spec.matlab} ({spec.name}): {raw.shape[0]} frames n_tx={raw.shape[1]} "
          f"patches={num_patches} {recon} -> IQ {iq.shape} "
